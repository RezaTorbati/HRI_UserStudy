...
class Subroutines:
    def __init__(self, remote):
        ...
        self.routines = ['connect_to_cube','pick_up_cube', "drive_to_charger"]

    def connect_to_cube(self):
        self.robot.world.connect_cube()
        if self.robot.world.connected_light_cube:
            self.remote.connected_to_cube = True
        else:
            self.remote.connected_to_cube = False
...
class RemoteControlVector:
    ...
    def __init__(self, robot):
        self.math_answer = ""
        self.math_points = 0
        self.math_df = pd.read_csv('questions.csv')
        self.math_df.reset_index()
        self.math_iter = self.math_df.iterrows()
        self.row = next(self.math_iter)[1]

        self.vector = robot
        self.connected_to_cube = False
        ...
        default_anims_for_keys = ["pick_up_cube",  # 0
                                  "connect_to_cube",  # 1
                                  "drive_to_charger",  # 2
                                  "pick_up_cube",  # 3
                                  "pick_up_cube",  # 4
    ...
    def answer_question(self, answer):
        try:
            if int(answer) == int(self.row['answer']):
                self.math_points += 1
        except Exception as e:
            print(e)
        self.row = next(self.math_iter)[1]
        print(self.row)
        print(self.math_points)
...
def get_connection_status():
    return str(flask_app.remote_control_vector.connected_to_cube)

def get_question():
    print('runnin 2')
    return f"{flask_app.remote_control_vector.row['term1']} * {flask_app.remote_control_vector.row['term2']} = "

def get_score():
    print('runnin')
    return f"{flask_app.remote_control_vector.math_points}"
            ...
            <table>
                <tr>
                    ...
                    <td valign=top>
                        ...
                        <b>Connected to Cube</b> : <label id="statusId">""" + get_connection_status() + """</label>
                    ...
                    <td width=30></td>
                    <td valign=top>
                    <h2>Subroutine key mappings:</h2>
                    """ + get_anim_sel_drop_downs() + """<br>

                    <h2>Extra Points Problems</h2>
                    <b><label id="questionId">""" + get_question() + """</label></b> : Answer <input type="text" name="mathText" id="mathTextId" value=\"""" + flask_app.remote_control_vector.math_answer + """\" onchange=handleTextInput(this)><br>
                    <b>Score</b> :  <label id="scoreId">""" + get_score() + """</label>
            ...
            <script type="text/javascript">
                ...
                function handleKeyActivity (e, actionType)
                {
                    ...
                    if (actionType=="keyup")
                    {
                        ...
                        {
                            //enterClicked()
                            document.getElementById("questionId").innerHTML = "Test";
                            document.getElementById("scoreId").innerHTML = "test2"//""" + get_score() + """;
                        }
                    }
